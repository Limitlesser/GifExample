...
public class GifImageLoader {
    ...
    private GifDrawable newGifDrawable(byte[] bytes) {
    ...
    public void displayGif(final ImageView imageView, final String url, final Options options, final LoadListener listener) {
        ...
        Observable.concat(loadFromMemory(url),
                ...
                .first(bytes -> bytes != null)
                .map(this::newGifDrawable)
    ...
    private Observable<byte[]> loadFromMemory(final String url) {
        return Observable.create((Subscriber<? super byte[]> subscriber) -> {
    ...
    private Observable<byte[]> loadFromDisk(final String url) {
        return Observable.create((Subscriber<? super byte[]> subscriber) -> {
    ...
    private Observable<byte[]> loadFromNetwork(final String url, final Options options) {
        return Observable.create((Subscriber<? super byte[]> subscriber) -> {
            try {
